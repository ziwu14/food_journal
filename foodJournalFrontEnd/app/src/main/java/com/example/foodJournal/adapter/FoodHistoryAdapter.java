...
public class FoodHistoryAdapter extends RecyclerView.Adapter<FoodHistoryAdapter.FoodHistoryViewHolder> {
    ...
    public FoodHistoryAdapter(Context context, ArrayList<String[]> dataList){
        mContext = context;
        mDataList = dataList;
    }

    @Override
    public FoodHistoryViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        LayoutInflater inflater = LayoutInflater.from(mContext);
        View view = inflater.inflate(R.layout.item_food_history, parent, false);
    ...
    public class FoodHistoryViewHolder extends RecyclerView.ViewHolder {
        ...
        public FoodHistoryViewHolder(View itemView) {
            super(itemView);//extend RecyclerView.ViewHolder

            name = itemView.findViewById(R.id.item_food_history_name);
            amount = itemView.findViewById(R.id.item_food_history_amount);
            calorie = itemView.findViewById(R.id.item_food_history_calorie);
            time = itemView.findViewById(R.id.item_food_history_time);

            delBtn = itemView.findViewById(R.id.item_food_history_deleteBtn);
            delBtn.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    int position = getAdapterPosition();
                    if (position != RecyclerView.NO_POSITION) {
                        mListerner.onItemDeleteClick(position);//remove from database
                        removeData(position);//remove from recycler view
                    }
                }
            });
